...
#define BLOCK_SIZE 4096  // in unit of byte, the size of one data block
#define THREAD_SIZE 128  // in unit of datatype, the size of the thread block, so as the size of symbols per iteration
#define WINDOW_SIZE 128  // in unit of datatype, maximum 255, the size of the sliding window, so as the maximum match length
#define INPUT_TYPE float // define input type, since c++ doesn't support runtime data type defination
// #define DEBUG
